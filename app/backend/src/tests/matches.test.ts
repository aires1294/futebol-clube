...
describe('GET /matches', () => {
    ...
    describe('quando a requisição da certo', () => {
        ...
        //     expect(chaihtppResponse.body).to.equal({ "message": "Finished" })

        // })

        it('deve retornar um status 200  ao editar uma partida /matches/:id', async () => {
            let chaihtppResponse: Response;
    ...
    //         expect(chaihtppResponse.body).to.equal({ "message": "Token not found" })
    ...
    //         expect(chaihtppResponse.status).to.equal(401)
    //         expect(chaihtppResponse.body).to.equal({ "message": "Token must be a valid token" })
